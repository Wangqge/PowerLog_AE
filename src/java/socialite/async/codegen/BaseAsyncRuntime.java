...
public abstract class BaseAsyncRuntime implements Runnable {
    ...
    protected void createThreads() {
        ...
        if (asyncConfig.getEngineType() == AsyncConfig.EngineType.SYNC)// For Sync/Semi-Async mode, the barrier is required
        ...
        if(asyncConfig.getEngineType() == AsyncConfig.EngineType.BLOCK_ASYNC){
            if(checkerThread==null)
                throw new AssertionError("create BlockSend before call createThreads");
            barrier = new CyclicBarrier(asyncConfig.getThreadNum(), blocksend);
        }
...
//                            }
                    ...
                    if (AsyncConfig.EngineType.BLOCK_ASYNC==asyncConfig.getEngineType()){
                        barrier.await();
               //         L.info("finish one round");
                    }
...
//                }
    ...
    protected  abstract class BlockSend extends Thread{
        protected BlockSend(){
