...
query = MyriaQuery.submit(
    """
E = load("hdfs://master:9000/experiment/live/edge_sssp.txt", csv(schema(src:int, dst:int, weight:int), delimiter="\t"));
INIT_SSSP = load("hdfs://master:9000/experiment/live/node_sssp.txt", csv(schema(src:int, dist:int), delimiter="\t"));
do
