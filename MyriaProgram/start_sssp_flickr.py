...
query = MyriaQuery.submit(
    """
E = load("hdfs://master:9000/experiment/flickr/edge_sssp.txt", csv(schema(src:int, dst:int, weight:int), delimiter="\t"));
INIT_SSSP = load("hdfs://master:9000/experiment/flickr/node_sssp.txt", csv(schema(src:int, dist:int), delimiter="\t"));
do
