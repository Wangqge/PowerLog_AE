...
public class katz {
    ...
    // 0       1             2    	3   	 4  	 5
    //single node-count edge-path  node-path parameter  iter-num
    //dist   node-count   edge-path node-path parameter  iter-num
    ...
    public static void main(String[] args) throws FileNotFoundException, InterruptedException {
        STGroup stg = new MySTGroupFile(PageRank.class.getResource("katz.stg"),
                "UTF-8", '<', '>');
        stg.load();
        if (args[0].equals("single")) {
            ...
            st.add("N", nodeCount);
            st.add("EDGEPATH", args[2]);//web-BerkStan_fix
	    st.add("NODEPATH", args[3]);
            st.add("P", Double.parseDouble(args[4]));//web-BerkStan_fix
...
//            System.out.println(vals[0]);
	...
	System.out.println(args[2]+" "+args[3]);
            st.add("EDGEPATH", args[2]);
	    st.add("NODEPATH", args[3]);
            st.add("P", Double.parseDouble(args[4]));
            String init = st.render();
            System.out.println(init);
            MasterNode.startMasterNode();
//            while (!MasterNode.getInstance().allOnLine())//waiting workers online
//                Thread.sleep(100);
