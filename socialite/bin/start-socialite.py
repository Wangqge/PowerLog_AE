...
if sys.argv[1] == 'start':
    command_line = '''
   /home/wangqg/jdk1.8.0_161/bin/java -Xmx26G -ea \\
    ...
    prog_args = 'dist %d %s %s %d'%(875713,'hdfs://master:9000/edge.txt','hdfs://master:9000/edge.txt',20)
    command_line = '''
    /home/wangqg/jdk1.8.0_161/bin/java -Xmx20G -ea \\
    -Dsocialite.worker.num=%d \\
    -Dsocialite.output.dir=%s/gen \\
    -Dsocialite.master=%s \\
    -Dsocialite.port=50100 \\
    ...
    for worker_host_name in WORKER_HOSTNAME_LIST:
        command_line = '''ssh -n %s@%s "/home/wangqg/jdk1.8.0_161/bin/jps|grep WorkerNode|awk '{print $1}'|xargs kill -9 2> /dev/null"''' % (
