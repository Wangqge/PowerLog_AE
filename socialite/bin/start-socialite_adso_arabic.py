...
if sys.argv[1] == 'start':
   ...
   %s/bin/java -Xmx26G -ea \\
    ...
    prog_args = 'dist %d %s%s%s %s%s%s %d %d'%(22744080,'hdfs://',MASTER_HOSTNAME,':9000/experiment/arabic/edge.txt','hdfs://',MASTER_HOSTNAME,':9000/experiment/arabic/node_adsorption.txt',200,40)
    #prog_args = 'dist %d %s %s %d %d'%(int(sys.argv[2]),sys.argv[3],sys.argv[4],int(sys.argv[5]),int(sys.argv[6]))
    command_line = '''
    %s/bin/java -Xmx26G -ea \\
    -Dsocialite.worker.num=%d \\
    -Dsocialite.output.dir=%s/gen \\
